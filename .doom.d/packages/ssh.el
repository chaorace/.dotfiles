...
(defcustom ssh-directory-tracking-mode 'nil
...
(cond
 ((null ssh-mode-map)
  ...
  (define-key ssh-mode-map "\C-c\C-c" 'cisco-send-Ctrl-C)
  ...
  (define-key ssh-mode-map "\C-i" 'cisco-autocomplete)))
...
(defun cisco-get-input ()
  (buffer-substring-no-properties
   (process-mark (get-buffer-process (current-buffer)))
   (point-max)))

(defun cisco-sent-silent (input)
